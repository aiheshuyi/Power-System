...
// 获取时间范围选项（基于实际数据范围）
export const getTimeRangeOptions = (data = []) => {
  const options = [];
...
// 获取实际数据的日期范围
export const getActualDataRange = (data) => {
  if (data.length === 0) {
...
// 获取可用的年份、月份、季度选项
export const getAvailableTimeOptions = (data) => {
  if (data.length === 0) {
    return {
...
export const calculateStats = (data) => {
  ...
  const stats = features.map((feature) => {
    const values = data.map((item) => item[feature]).filter((v) => !isNaN(v));
...
// 检查数据文件是否存在
export const checkDataFileExists = async () => {
  try {
...
// 获取数据文件信息
export const getDataFileInfo = async () => {
  try {
    const response = await fetch('/22-25_All.csv', { method: 'HEAD' });
    if (response.ok) {
...
// 检测文件编码
export const detectFileEncoding = (uint8Array) => {
  // 检查BOM
...
// 转换编码
export const convertEncoding = (uint8Array, encoding) => {
  try {
...
// 验证文件内容
export const validateFileContent = (csvText) => {
  const errors = [];
