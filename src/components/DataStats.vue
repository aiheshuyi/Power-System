  ...
  return [
    {
      ...
      key: 'feature',
      width: 200,
      customRender: ({ text }: { text: string }) => featureLabels[text] || text
    },
    {
      title: '数据点数',
      dataIndex: 'count',
      ...
      customRender: ({ text, record }: any) => {
        const total = totalStats.value.features.find((f: any) => f.feature === record.feature);
        return h('span', null, [
          text,
          total
            ? h(
                'span',
                { style: { fontSize: '12px', color: '#999', marginLeft: 4 } },
                `/${total.count}`
              )
            : null
        ]);
